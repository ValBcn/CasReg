...
class Unet(nn.Module):
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, kernel_size=7, feat_mult=1):
        super().__init__()
        """
        Parameters:
        ...
        for i, nf in enumerate(self.dec_nf[:len(self.enc_nf)]):
            ...
            #self.uparm.append(ConvNextBlock(nf,nf))
            prev_nf = nf
        ...
        self.norm = nn.LayerNorm(self.enc_nf[-1], eps=1e-6)
...
class CasReg(nn.Module):
    """
    Cascaded networks for unsupervised nonlinear registration between two images.
    """
    def __init__(self,
                 inshape,
                 n_casc=2,
        ...
        self.unet_feat_mult = unet_feat_mult
        # internal flag indicating whether to return flow or integrated warp during inference
        self.training = True
        self.mode = mode
    ...
    def forward(self, x: torch.Tensor):
        ...
        for casc in range(self.nb_cascades):
            ...
            max_flow_list.append(round(flow_field.max().item(),3))
            ...
            flow_list.append(flow_field)
        print("max flow : ", max_flow_list)
        # warp image with flow field
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        return nnf.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)
